...
WHERE
    ...
    AND (
        ...
        ((prf.prfid LIKE 'IRT MM%' OR prf.prfid LIKE 'LIQ MM%' OR prf.prfid LIKE 'LIQ BI%' OR prf.prfid LIKE 'ALM Repo%')
         AND p.ptyid NOT IN ('10000000001_BANKMANDIRI', 'Mandiri TRS HO'))
        OR
        /*-- Kondisi untuk transaksi dengan port IRT MM Depo Loan Repo RR dengan counterparty Bank Mandiri atau Mandiri TRS HO atau TRANSAKSI INTERNAL*/
        (prf.prfid LIKE 'IRT MM IDR%' AND p.ptyid IN ('10000000001_BANKMANDIRI', 'Mandiri TRS HO'))
    )

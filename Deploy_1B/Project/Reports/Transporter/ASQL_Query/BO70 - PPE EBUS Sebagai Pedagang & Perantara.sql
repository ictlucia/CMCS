...
WHERE
    ...
    i.instype = 'Curr' AND
    CONVERT('date', p.day, '%B %Y') = '{{monthUsed}}' AND
...
WHERE 
    ...
    CONVERT('date', t.time, '%B %Y') = '{{monthUsed}}' AND
...
SELECT 
    ...
    t.transactionType,
    t.rowDesc, 
    SUM((convert('string', t.insid, 2) IN ('VR', 'FR') OR convert('string', t.insid, 3) IN ('ORI', 'SPN', 'ROI')) AND convert('string', t.insid, 4) NOT IN ('SPNS') ? abs(t.nominalIDR) : 0) 'sbn_val',
    SUM((convert('string', t.insid, 2) IN ('VR', 'FR') OR convert('string', t.insid, 3) IN ('ORI', 'SPN', 'ROI')) AND convert('string', t.insid, 4) NOT IN ('SPNS') ? 1 : 0) 'sbn_freq',
    SUM((
    ...
    SUM((convert('string', t.insid, 2) IN ('SR') OR convert('string', t.insid, 3) IN ('PBS') OR convert('string', t.insid, 4) IN ('SPNS') OR convert('string', t.insid, 6) IN ('INDOIS')) ? abs(t.nominalIDR) : 0) 'sbsn_val',
    SUM((convert('string', t.insid, 2) IN ('SR') OR convert('string', t.insid, 3) IN ('PBS') OR convert('string', t.insid, 4) IN ('SPNS') OR convert('string', t.insid, 6) IN ('INDOIS')) ? 1 : 0) 'sbsn_freq',
    SUM(t.insId LIKE '%SUKUK%' ? abs(t.nominalIDR) : 0) 'sukuk_val',
    SUM(t.insId LIKE '%SUKUK%' ? 1 : 0) 'sukuk_freq',
    SUM((convert('string', t.insid, 3) IN ('SBK')) ? abs(t.nominalIDR) : 0) 'other_val',
    SUM((convert('string', t.insid, 3) IN ('SBK')) ? 1 : 0) 'other_freq',

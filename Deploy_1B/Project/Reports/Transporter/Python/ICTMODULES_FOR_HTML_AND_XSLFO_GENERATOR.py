...
class HTMLGenerator:
    ...
    def create_base_html_content(self, title, additional_styles=''):
        ...
        """ + additional_styles + """
            ...
            <body>
            <center><h1 class='title'><u>""" + str(title) +  """</u></h1></center>"""
    ...
    def prepare_html_table(self, html_content, header_list, row_style='', header_style='', table_styling=''):
    ...
    def add_subtitle_separator(self, html_content, title, style_string='', attribute_string=''):

        html_content += "<tr>" + f"<td style='{style_string}' {attribute_string}>" + title + "</td>" + "</tr>"

        return html_content

    def create_html_file(self, html_content, file_path, file_name, current_date, open_html, folder_with_file_name=False):
...
class XSLFOGenerator:
    ...
    def add_xsl_fo_table_header(self, xsl_fo_content, header_list, header_styling="padding='8pt' border-width='1px' border-style='solid'"):
        ...
        for header in header_list:
            ...
            xsl_fo_table += f"""<fo:table-cell {header_styling}><fo:block>""" +str(header)+"""</fo:block></fo:table-cell>
            """
    ...
    def add_xsl_subtitle_separator(self, xsl_content, title, style_string='', colspan=1):

        xsl_content += """<fo:table-row>""" + f"""<fo:table-cell border-width="1px" border-style="solid" padding="8pt" number-columns-spanned="{colspan}"><fo:block {style_string}>""" + title +"""</fo:block></fo:table-cell>""" + """</fo:table-row>
        """

        return xsl_content

    def add_xsl_data_row(self, xsl_content, row_data, cell_styling="border-width='1px' border-style='solid' padding='8pt'", block_styling=''):
        ...
        for row in row_data:        
            ...
            for cell in row:
                ...
                xsl_content += f"""<fo:table-cell {cell_styling}><fo:block {block_styling}>""" +str(cell)+"""</fo:block></fo:table-cell>
                """
    ...
    def prepare_xsl_row(self, xsl_content):
    ...
    def add_xsl_column(self, xsl_content, data, cell_style="border-width='1px' border-style='solid' padding='8pt'", block_style=''):

        xsl_content +=f"<fo:table-cell {cell_style}><fo:block {block_style}>" + str(data) + "</fo:block></fo:table-cell>"
    ...
    def create_xsl_fo_file(self, file_name, file_path, xsl_fo_content, current_datetime, folder_with_file_name=False):
...
def create_html_file(file_name, file_path, list_table, title, current_datetime, folder_with_file_name=False, open_html=True) :
...
def create_xsl_fo_table(list_title, list_row):
    ...
    for i in list_title:
        ...
        xsl_fo_table += """<fo:table-cell padding="8pt" border-width="1px" border-style="solid"><fo:block>""" +str(i)+"""</fo:block></fo:table-cell>
        """
    ...
    for i in list_row:        
        ...
        for j in i:
            ...
            xsl_fo_table += """<fo:table-cell border-width="1px" border-style="solid" padding="8pt"><fo:block>""" +str(j)+"""</fo:block></fo:table-cell>
            """
...
def get_current_date(separator):
    ...
    current_date = now.strftime("%d"+separator+"%m"+separator+"%Y")
...
def get_current_hour(separator):
    ...
    current_hour = now.strftime("%H"+separator+"%M"+separator+"%S")
...
def get_current_report_datetime(separator):
    ...
    current_datetime = now.strftime("%d"+separator+"%m"+separator+"%Y"+" "+"%H"+":"+"%M"+":"+"%S"+" UTC"+tz_format)

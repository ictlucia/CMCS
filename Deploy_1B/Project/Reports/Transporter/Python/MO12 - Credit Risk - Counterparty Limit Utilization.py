...
def cpty_trades_calc(cpty_name, calc_space, col_name):
    trades = cpty_util_calc_dicts[cpty_name]['All Trades']
    total_val = 0.0
    for trd in trades:
        trd_val = get_trading_sheet_column_value(trd, calc_space, col_name)
        total_val += trd_val
    cpty_util_calc_dicts[cpty_name]['Utilization'] = total_val
    return total_val
...
ael_gui_parameters={'runButtonLabel':'&&Run',
                    ...
                    'windowCaption':'MO12 - Credit Risk - Counterparty Limit Utilization'}
#settings.FILE_EXTENSIONS
ael_variables=[
['report_name','Report Name','string', None, 'MO12 - Credit Risk - Counterparty Limit Utilization', 1,0],
['file_path', 'Folder Path', getFilePathSelection(), None, getFilePathSelection(), 1, 1],
['output_file','Secondary Output Files','string', ['.pdf', '.xls'], '.pdf',0,1, 'Select Secondary Extensions Output'],
['compliance_rule_name', 'Compliance Rule', 'string', compliance_rule_list('PositionAndRiskControl', 'Pre-Deal'), 'TOT_BGRP_USD', 0, 0, 'Select Compliance rule Name']
]  
def ael_main(parameter):
    ...
    for pty in party_list:
        ...
        all_ports.append( pty_obj )



    
    
    titles = ['Customer Name', 'Group ID', 'Limit', 'Utilisasi Credit Equivalent', '1 Day Change', '% Utilization', 'Status Limit', 'Limit Expiry', 'Status Limit Expiry', 'Notes']
    

    columns = ['Limit', 'Utilisasi', '1 Day Change', '% Utilization']
    
    
    country_name = 'Country United State'
    ...
    for port in all_ports:
        ...
        util_val_obj = get_rule_value_by_rule_name(obj, rule_name)
        prev_util_val_obj = get_prev_day_rule_val_by_rule_name(obj, rule_name)

        curr = get_currency_by_rule_name(rule_name)
        cross_rate = get_cross_rate_by_rule_val(util_val_obj, curr)

        temp_row.append(obj.Id()) #1
        
        temp_row.append( thousand_dot_sep( get_threshold_limit_by_rule_name(obj, rule_name) * cross_rate ) )
        ...
        try:
            temp_row.append( get_expiry_status( end_date, today ) ) #8
        except:
            temp_row.append( '-' ) #8
        ...
        temp_row.insert( 5, percent_val ) #5
        if len(tv_list) > 0:
            temp_row.insert( 6, limit_indicator( tv_list, util_val ) ) #6
        else:
            temp_row.insert( 6, 'Green' ) #6
        temp_row.insert( 7, end_date ) #7
        

        
        temp_row.append( get_last_diary_note_by_rule_name(obj, rule_name) ) #9
    ...
    #'Limit', 'Utilisasi Credit Equivalent', '1 Day Change', '% Utilization'

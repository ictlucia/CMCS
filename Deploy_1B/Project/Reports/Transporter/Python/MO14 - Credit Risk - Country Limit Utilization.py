...
def get_countries_by_ael_sql(pty_list):
    #country_list = [ country for country in acm.FChoiceList.Select('list="Country of Risk" and name<>None') ]
    for pty in pty_list:
        country = acm.FChoiceList[ pty[0] ]
...
def cty_trades_calc(cty_name, calc_space, col_name):
    trades = country_util_calc_dicts[cty_name]['All Trades']
    total_val = 0.0
    for trd in trades:
        trd_val = get_trading_sheet_column_value(trd, calc_space, col_name)
        total_val += trd_val
    country_util_calc_dicts[cty_name]['Utilization'] = total_val
    return total_val
...
['compliance_rule_name', 'Compliance Rule', 'string', compliance_rule_list('PositionAndRiskControl', 'Pre-Deal'), 'LOG_CTY', 0, 0, 'Select Compliance rule Name']
]  
def ael_main(parameter):
    ...
    party_list = ael.asql("""
    Select c.seqnbr 
    from ChoiceList c, AppliedRule a, ComplianceRule cr 
    where a.compliance_rule = cr.seqnbr  
    and a.target_seqnbr= c.seqnbr
    and cr.name = '{}' 
    and inactive = 'No'
    """.format(rule_name))[1][0]
    ...
    for i in country_list:
        #if i is not None:
        all_ports = all_ports + get_party_data_by_ael_sql(i)
    ...
    for port in all_ports:
        ...
        if len(tv_list) > 0:
            ...
            temp_row.insert( 5, 'Green' )
        temp_row.insert( 6, end_date )
        
        temp_row.append( get_last_diary_note_by_rule_name(obj, rule_name) )

...
def getPNLTrade(tradenumber, calcspace):
...
def ael_main(parameter):
    ...
    # QUERY: Portfolio = TAS
    query = ael.asql(f"""SELECT t.trdnbr, t.premium,p.prfid 
                        FROM trade t, portfolio p, portfoliolink pl 
                        WHERE p.prfnbr=pl.member_prfnbr 
                        AND (pl.owner_prfnbr={str(get_prfnbr('TAS'))})
    ...
    for i in range(length):
        if query[1][0][i][2] == prfid:
            theoreticalval = getPNLTrade(query[1][0][i][0], calcSpace)

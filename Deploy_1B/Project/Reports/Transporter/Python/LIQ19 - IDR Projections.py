...
"Interbank Placement" : {
    "Product Type" : ["DL"],
    "Category" : ["CL", "CMP", "OVP", "SHARI", "MD"]
    },
"Interbank Taken" : {
    "Product Type" : ["DL"],
    "Category" : ["CL", "CMT", "OVT", "BLT"]
    },
"LF" : {
    "Product Type" : ["DL"],
    "Category" : ["LF"]
...
"Surat Berharga Dimiliki" : {
    "Product Type" : ["BOND", "SBI"],
    "Category" : ["IDBI", "SUKBI", "IDSD", "CBIDR", "ORI", "SR", "SBBI", "SBK", "SPN", "SPNS", "FR", "VR", "PBS", "NCD", "OPT", "FWD"]
    },
"Coupon SB Dimiliki" : {
    "Product Type" : ["BOND", "SBI"],
    "Category" : ["IDBI", "SUKBI", "IDSD", "CBIDR", "ORI", "SR", "SBBI", "SBK", "SPN", "SPNS", "FR", "VR", "PBS", "NCD", "OPT", "FWD"]
    ...
    for i in all_trades[column_name]:
        ...
        try:
            ...
            if (filter_currency == temp_curr or filter_currency == temp_instrument) and temp_category in list_category and time_range == 0 and status_opt_key_3:
                ...
                if column_name not in list_else:
                ...
                elif title_type == "Maturity" and column_name == "DF" and str(i.InstrumentSubType()) == "Deposit/Loan" and i.Premium() >= 0 and temp_curr == 'IDR' and filter_currency == 'IDR' : # Tabel Kiri, DF
                    total_result += get_projected_from_money_flow(i, valueday, cashflow_type)
                    status_empty_transaction = False
                elif title_type == "Over" and column_name == "DF" and str(i.InstrumentSubType()) == "Deposit/Loan" and i.Premium() >= 0 and temp_curr == 'IDR' and filter_currency == 'IDR': # Tabel Kanan, DF
                    total_result += i.Premium()
                    status_empty_transaction = False
                elif title_type == "Maturity" and column_name == "LF" and str(i.InstrumentSubType()) == "Deposit/Loan" and i.Premium() < 0 and temp_curr == 'IDR' and filter_currency == 'IDR': # Tabel Kiri, LF
                    total_result += get_projected_from_money_flow(i, valueday, cashflow_type)
                    status_empty_transaction = False
                elif title_type == "Over" and column_name == "LF" and str(i.InstrumentSubType()) == "Deposit/Loan" and i.Premium() < 0 and temp_curr == 'IDR' and filter_currency == 'IDR': # Tabel Kanan, LF
                    total_result += i.Premium()
                    status_empty_transaction = False
                    ...
                    total_result += get_value_for_swap_trade(i, filter_currency)
                    status_empty_transaction = False
                elif title_type == "Maturity" and column_name == "Repo BI" and "Repo/Reverse" in str(i.InstrumentSubType()) and i.StartCash() < 0: # Tabel Kiri, Repo BI
                    total_result += get_sum_cashflow_from_trade(i)
                    status_empty_transaction = False
                elif title_type == "Over" and column_name == "Repo BI" and "Repo/Reverse" in str(i.InstrumentSubType()) and i.StartCash() < 0: # Tabel Kanan, Repo BI
                    total_result += i.StartCash()
                    status_empty_transaction = False
                elif title_type == "Maturity" and column_name == "Reverse Repo BI" and "Repo/Reverse" in str(i.InstrumentSubType()) and i.StartCash() >= 0: # Tabel Kiri, Reverse Repo BI
                    total_result += get_sum_cashflow_from_trade(i)
                    status_empty_transaction = False
                elif title_type == "Over" and column_name == "Reverse Repo BI" and "Repo/Reverse" in str(i.InstrumentSubType()) and i.StartCash() >= 0: # Tabel Kanan, Reverse Repo BI
                    total_result += i.StartCash()
                    status_empty_transaction = False
                elif title_type == "Maturity" and column_name == "Repo Interbank" and "Repo/Reverse" in str(i.InstrumentSubType()) and i.StartCash() < 0: # Tabel Kiri, Repo Interbank
                    total_result += get_sum_cashflow_from_trade(i)
                    status_empty_transaction = False
                elif title_type == "Over" and column_name == "Repo Interbank" and "Repo/Reverse" in str(i.InstrumentSubType()) and i.StartCash() < 0: # Tabel Kanan, Repo Interbank
                    total_result += i.StartCash()
                    status_empty_transaction = False
                elif title_type == "Maturity" and column_name == "Reverse Repo Interbank" and "Repo/Reverse" in str(i.InstrumentSubType()) and i.StartCash() >= 0: # Tabel Kiri, Reverse Repo Interbank
                    total_result += get_sum_cashflow_from_trade(i)
                    status_empty_transaction = False
                elif title_type == "Over" and column_name == "Reverse Repo Interbank" and "Repo/Reverse" in str(i.InstrumentSubType()) and i.StartCash() >= 0: # Tabel Kanan, Reverse Repo Interbank
                    total_result += i.StartCash()
                    status_empty_transaction = False
                    ...
                    total_result += i.Premium()
                    status_empty_transaction = False
                elif title_type == "Maturity" and column_name == "Coupon SB Dimiliki" : # Tabel Kiri, Coupon SB Dimiliki (Hanya Maturity)
                    total_result += get_projected_from_money_flow(i, valueday, cashflow_type)
                    status_empty_transaction = False
                elif title_type == "Maturity" and column_name == "Coupon SB Terbit" : # Tabel Kiri, Coupon SB Terbit (Hanya Maturity)
                    total_result += get_projected_from_money_flow(i, valueday, cashflow_type)
                    status_empty_transaction = False
...
ael_gui_parameters={'runButtonLabel':'&&Run',
                    ...
                    'windowCaption':'Cashflow Projection'}
#settings.FILE_EXTENSIONS
ael_variables=[
['report_name','Report Name','string', None, 'Cashflow Projection', 1,0],
['from_date','From (Start Date)','string', [acm.Time.DateToday(), "1M", "6M", "1Y", next_month, next_year], today, 1,0],
['to_date','To (End Date)','string', ["1M", "6M", "1Y", next_month, next_year], "5D", 1,0],
['filter_currency','Currency','string', None, 'IDR', 1,0, "Select to Filter the Currency", None, 0],
...
def ael_main(parameter):
    ...
    title_ins_maturity = ['Interbank Placement', 'Interbank Taken', 'FX/FW', 'SWAP', 'DF', 'LF', 'Term Deposit BI', 'Repo BI', 'Reverse Repo BI',
    'Repo Interbank', 'Reverse Repo Interbank', 'Surat Berharga Dimiliki', 'Coupon SB Dimiliki', 'Surat Berharga Terbit', 'Coupon SB Terbit','Reksadana', 'CCS', 'IRS']
    title_ins_over = ['Interbank Placement', 'Interbank Taken', 'SWAP', 'Repo BI', 'Reverse Repo BI', 'Repo Interbank', 'Reverse Repo Interbank', 'Surat Berharga Dimiliki', 'Coupon SB Dimiliki', 'Surat Berharga Terbit', 'CCS', 'IRS', 'Term Deposit BI']
    ...
    print(f'IDR Projections, It took {end_time- start_time: 0.2f} second(s) to complete.')

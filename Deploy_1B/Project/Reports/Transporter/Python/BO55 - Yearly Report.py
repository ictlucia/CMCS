...
ael_variables=[
['report_name','Report Name','string', None, 'Yearly Report', 1,0],
['from_year','From (Start Year)','int', [i for i in range(1970, current_year + 1)], 2020, 1,0],
['to_year','To (End Year)','int', [i for i in range(1970, current_year + 1)], current_year, 1,0],
...
def ael_main(parameter):
    ...
    if from_year > to_year:
        ...
        list_year = [i for i in range(from_year, to_year+1)]
        ...
        for year in list_year:
            ...
            # MONTH ITERATION
            for i in range(1, len(list_month)+2):
                temp_freq, temp_volume = [], []
                if i < len(list_month)+1: #Monthly Part
                    ...
                    for type in ('FREQUENCY', 'VOLUME'):
                        if type == 'FREQUENCY':
                            ...
                            list_volume.append(temp_volume)
            year_data[year] = [reple_freq, list_freq, reple_volume, list_volume]
        i = 1
        for data in year_data.values():        
            for k in range(1, len(list_month)+2):
                rows[i].append(data[0][k-1]) #reple_freq
                rows[i].extend(data[1][k-1]) #list_freq
                rows[i].append(data[2][k-1]) #reple_volume
                rows[i].extend(data[3][k-1]) #list_volume
                i += 1
        ...
        for year in list_year:
            table_html = table_html.replace('<td> {}</td>'.format(str(year)), '<td rowspan="12" style="background-color:#DDEBF6;font-weight:bold"> {}</td>'.format(str(year)))
        for row in rows:
    ...
    <fo:table-row>
        ...
        '<fo:table-cell border-width="1px" border-style="solid" padding="8pt" font-weight="bold" background-color="#DDEBF6" number-columns-spanned="1" number-rows-spanned="12"><fo:block> {}</fo:block></fo:table-cell>'.format(str(year))
        )
        for row in rows:
            row_data = [str(r) for r in row[1:]] if row[1] == list_month[0] else row

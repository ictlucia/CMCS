...
def ael_main(parameter):
    ...
    for each_port in rule_monthly.AppliedRules():
        if each_port.Target().Name() == 'IRT':
            limit_irt_monthly = each_port.ThresholdValues().First().Value()
    ...
    for each_port in rule_yearly.AppliedRules():
        if each_port.Target().Name() == 'IRT':
            limit_irt_yearly = each_port.ThresholdValues().First().Value()
    ...
    html_content = html_gen.add_cell_data(html_content,"IRT","style='font-weight:bold'")
    xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,"IRT",'font-weight="bold"')
    ...
    html_content = html_gen.add_cell_data(html_content,f"{limit_irt_monthly:,}","style='font-weight:bold'")
    xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,f"{limit_irt_monthly:,}",'font-weight="bold"')
    
    if limit_irt_monthly < 0 and total_category_monthly > 0:
        total_percent_irt_monthly = 0
    else :
        total_percent_irt_monthly = round(total_category_monthly/limit_irt_monthly*100,2)
        
    html_content = html_gen.add_cell_data(html_content,f"{abs(total_percent_irt_monthly):,}" + ' %',"style='font-weight:bold'")
    xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,f"{abs(total_percent_irt_monthly):,}" + ' %','font-weight="bold"')
    
    if total_percent_irt_monthly == 0:
        html_content = html_gen.add_cell_data(html_content,'','class="green"')
        xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,'','background-color="green"')  
        
    else:
        if (total_percent_irt_monthly >85 and total_percent_irt_monthly<100):
            html_content = html_gen.add_cell_data(html_content,'','class="yellow"')
            xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,'','background-color="yellow"')
        elif total_category_monthly>=limit_irt_monthly:
            html_content = html_gen.add_cell_data(html_content,'','class="green"')
            xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,'','background-color="green"')  
        else:
            html_content = html_gen.add_cell_data(html_content,'','class="red"')
    ...
    html_content = html_gen.add_cell_data(html_content,f"{limit_irt_yearly:,}","style='font-weight:bold'")
    xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,f"{limit_irt_yearly:,}",'font-weight="bold"')
    
    if limit_irt_yearly < 0 and total_category_yearly > 0:
        total_percent_irt_yearly = 0
    else :
        total_percent_irt_yearly = round(total_category_yearly/limit_irt_yearly*100,2)
        
    html_content = html_gen.add_cell_data(html_content,f"{abs(total_percent_irt_yearly):,}" + ' %',"style='font-weight:bold'")
    xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,f"{abs(total_percent_irt_yearly):,}" + ' %','font-weight="bold"')
    
    if total_percent_irt_yearly == 0:
        html_content = html_gen.add_cell_data(html_content,'','class="green"')
        xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,'','background-color="green"')  
        
    else:
        if (total_percent_irt_yearly >85 and total_percent_irt_yearly<100):
            html_content = html_gen.add_cell_data(html_content,'','class="yellow"')
            xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,'','background-color="yellow"')
        elif total_category_yearly>=limit_irt_yearly:
            html_content = html_gen.add_cell_data(html_content,'','class="green"')
            xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,'','background-color="green"')  
        else:
            html_content = html_gen.add_cell_data(html_content,'','class="red"')

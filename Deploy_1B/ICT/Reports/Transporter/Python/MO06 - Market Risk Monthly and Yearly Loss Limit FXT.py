...
ael_gui_parameters={'runButtonLabel':'&&Run',
                    ...
                    'windowCaption':'Montly & Yearly Limit Loss - FXT'}

ael_variables=[
['report_name','Report Name','string', None, 'MO06 - Monthly & Yearly Limit - FXT', 1,0],
...
def ael_main(parameter):
    ...
    for each_port in rule_monthly.AppliedRules():
        if each_port.Target().Name() == 'FXT':
            limit_fxt = each_port.ThresholdValues().First().Value()
            
        if 'FXT' in each_port.Target().Name() and each_port.Target().Compound()==False:
    ...
    for each_port in rule_yearly.AppliedRules():
        if each_port.Target().Name() == 'FXT':
            limit_fxt_yearly = each_port.ThresholdValues().First().Value()
    ...
    html_content = html_gen.create_base_html_content("MO06 - Monthly & Yearly Limit - FXT", title_style)
    xsl_fo_content = xsl_gen.prepare_xsl_fo_content("MO06 - Monthly &#38; Yearly Limit - FXT")
    ...
    html_content = html_gen.add_cell_data(html_content,"FXT","style='font-weight:bold'")
    xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,"FXT",'font-weight="bold"')
    ...
    html_content = html_gen.add_cell_data(html_content,f"{limit_fxt:,}","style='font-weight:bold'")
    xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,f"{limit_fxt:,}",'font-weight="bold"')
    
    if limit_fxt < 0 and total_category_monthly > 0:
        total_percent_fxt = 0
    else :
        total_percent_fxt = round(total_category_monthly/limit_fxt*100,2)
        
    html_content = html_gen.add_cell_data(html_content,f"{abs(total_percent_fxt):,}" + ' %',"style='font-weight:bold'")
    xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,f"{abs(total_percent_fxt):,}" + ' %','font-weight="bold"')
    
    if total_percent_fxt == 0:
        html_content = html_gen.add_cell_data(html_content,'','class="green"')
        xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,'','background-color="green"')  
        
    else:
        if (total_percent_fxt >85 and total_percent_fxt<100):
            html_content = html_gen.add_cell_data(html_content,'','class="yellow"')
            xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,'','background-color="yellow"')
        elif total_category_monthly>=limit_fxt:
            html_content = html_gen.add_cell_data(html_content,'','class="green"')
            xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,'','background-color="green"')  
        else:
            html_content = html_gen.add_cell_data(html_content,'','class="red"')
    ...
    html_content = html_gen.add_cell_data(html_content,f"{limit_fxt_yearly:,}","style='font-weight:bold'")
    xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,f"{limit_fxt_yearly:,}",'font-weight="bold"')
    
    if limit_fxt_yearly < 0 and total_category_yearly > 0:
        total_percent_fxt_yearly = 0
    else :
        total_percent_fxt_yearly = round(total_category_yearly/limit_fxt_yearly*100,2)
        
    html_content = html_gen.add_cell_data(html_content,f"{abs(total_percent_fxt_yearly):,}" + ' %',"style='font-weight:bold'")
    xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,f"{abs(total_percent_fxt_yearly):,}" + ' %','font-weight="bold"')
    
    if total_percent_fxt_yearly == 0:
        html_content = html_gen.add_cell_data(html_content,'','class="green"')
        xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,'','background-color="green"')  
        
    else:
        if (total_percent_fxt_yearly >85 and total_percent_fxt_yearly<100):
            html_content = html_gen.add_cell_data(html_content,'','class="yellow"')
            xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,'','background-color="yellow"')
        elif total_category_yearly>=limit_fxt_yearly:
            html_content = html_gen.add_cell_data(html_content,'','class="green"')
            xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,'','background-color="green"')  
        else:
            html_content = html_gen.add_cell_data(html_content,'','class="red"')

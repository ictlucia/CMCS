...
def NoneWatermarkValueConverter(util_val):
    res_util_val = util_val
    if res_util_val is None:
        return 0.0
    else:
        return res_util_val
...
DVNTL_default_rules = "DVNTL_GRP_GR_USD"
DV_default_rules = "DV_GRP_GR_USD"

ael_gui_parameters={'runButtonLabel':'&&Run',
                    'hideExtraControls': True,
                    'windowCaption':'MO12 - Credit Risk - Counterparty Limit Utilization DV'}
#settings.FILE_EXTENSIONS
ael_variables=[
['report_name','Report Name','string', None, 'MO12 - Credit Risk - Counterparty Limit Utilization DV', 1,0],
['file_path', 'Folder Path', getFilePathSelection(), None, getFilePathSelection(), 1, 1],
['output_file','Secondary Output Files','string', ['.pdf', '.xls'], '.pdf',0,1, 'Select Secondary Extensions Output'],
['ntl_compliance_rule_name', 'Notional Compliance Rule', 'string', compliance_rule_list('PositionAndRiskControl', 'Pre-Deal'), DVNTL_default_rules, 0, 1, 'Select Compliance rule Name'],
['crd_compliance_rule_name', 'Credit Equivalent Compliance Rule', 'string', compliance_rule_list('PositionAndRiskControl', 'Pre-Deal'), DV_default_rules, 0, 1, 'Select Compliance rule Name']
]
...
def ael_main(parameter):
    ...
    #party_list = [('CounterParty A',),('Counterparty A3',),('CounterParty A1_2',)]
    #party_list = [('Counterparty 2','United States',)]
    product_list = ['Option', 'IRS','CCS','Structured Product']
    ...
    for key in list( rule_set_dicts.keys() ):
        ...
        for port in all_ports:
            ...
            if obj in rule_set_dicts[key]['NTL Active Parties']:
                ...
                try:
                    util_val = NaN_to_zero( temp_util_val ) * cross_rate
                    util_val_res = util_val
                except:
                    util_val_res = 0.0

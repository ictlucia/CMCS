...
def get_curr_name():
    all_curr = acm.FCurrency.Select("")
    obj_curr = []
    for each_curr in all_curr:
        obj_curr.append(each_curr.Name())
    return obj_curr
...
def get_market_value_port(data,all_curr):
    list_data = []
    data_dict = {}
    for each_data in data:
        for each_curr in all_curr:
            if each_data[0] == "Trade " + each_curr:
...
def get_1DChange(qf_operation):
    ...
    for each_curr,wm_items in watermark_today.items():
        for each_curr2,wm_items2 in watermark_yesterday.items():
            if each_curr == each_curr2:
                oneday_change[each_curr] = wm_items[0] - wm_items2[0]
...
ael_gui_parameters={'runButtonLabel':'&&Run',
                    ...
                    'windowCaption':'Value at Risk per Currency'}

ael_variables=[
['report_name','Report Name','string', None, 'MO04 - Market Risk - Value at Risk per Currency', 1,0],
['file_path', 'Folder Path', getFilePathSelection(), None, getFilePathSelection(), 1, 1],
['csv_name','CSV File Name','string',None,'MO04 - Market Risk - Value at Risk per Currency',1,0], 
...
def ael_main(parameter):
    ...
    html_gen = HTMLGenerator()
    xsl_gen = XSLFOGenerator()
    all_curr = get_curr_name()
    all_data = get_items(file_path,csvname)
    market_value = get_market_value_port(all_data,all_curr)
    ...
    for each_curr,items in market_value.items():
        utilisasi_hvar = 0
        html_content = html_gen.open_table_row(html_content)
        xsl_fo_content = xsl_gen.prepare_xsl_row(xsl_fo_content)
        
        html_content = html_gen.add_cell_data(html_content,each_curr)
        xsl_fo_content = xsl_gen.add_xsl_column(xsl_fo_content,each_curr)
        ...
        #Historical VaR
        try:        
            utilisasi_hvar = watermark_today_hvar[each_curr][0]
        ...
        try:        
            onedaychange_hvar = net_watermark_hvar[each_curr]
        ...
        #Marginal VaR
        try:        
            utilisasi_marginal = watermark_today_marg[each_curr][0]
        ...
        try:        
            onedaychange_marginal = net_watermark_marg[each_curr]
        ...
        #Incremental VaR
        try:        
            utilisasi_incremental = watermark_today_inc[each_curr][0]
        ...
        try:        
            onedaychange_incremental = net_watermark_inc[each_curr]
        ...
        #Interest Rate VaR
        try:        
            utilisasi_ir = watermark_today_ir[each_curr][0]
        ...
        try:        
            onedaychange_ir = net_watermark_ir[each_curr]
        ...
        #FX VaR
        try:        
            utilisasi_fx = watermark_today_fx[each_curr][0]
        ...
        try:        
            onedaychange_fx = net_watermark_fx[each_curr]
        ...
        #Credit Risk VaR
        try:        
            utilisasi_cr = watermark_today_cr[each_curr][0]
        ...
        try:        
            onedaychange_cr = net_watermark_cr[each_curr]
        ...
        #Shortfall
        try:        
            utilisasi_sf = watermark_today_sf[each_curr][0]
        ...
        try:        
            onedaychange_sf = net_watermark_sf[each_curr]

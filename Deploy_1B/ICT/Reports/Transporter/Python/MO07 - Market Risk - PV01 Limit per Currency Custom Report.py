...
usd_trades = [trd for trd in all_trades if trd.Currency().Name() == 'USD']
idr_trades = [trd for trd in all_trades if trd.Currency().Name() == 'IDR']
eur_trades = [trd for trd in all_trades if trd.Currency().Name() == 'EUR']
sgd_trades = [trd for trd in all_trades if trd.Currency().Name() == 'SGD']
...
ael_gui_parameters={'runButtonLabel':'&&Run',
                    ...
                    'windowCaption':'MO07 - Market Risk - PV01 Limit per Currency'}
#settings.FILE_EXTENSIONS
ael_variables=[
['report_name','Report Name','string', None, 'MO07 - Market Risk - PV01 Limit per Currency', 1,0],
...
def ael_main(parameter):
    ...
    qf_name = "MO07 Currency"
    
    #all_ports = []

    all_ports = list(qf_trade_dicts.keys())
    ...
    for port in all_ports:
        ...
        cur_trades = qf_trade_dicts[port]
        cur_instruments = get_portfolio_instruments(port)
        #if qf is not None:
       
            
        mkt_value = port_trades_calculation(cur_trades, calc_space, 'Portfolio Market Value')
        mkt_value_res = thousand_dot_sep( mkt_value )
        
        pv01_val = port_trades_calculation(cur_trades, calc_space, 'Portfolio Delta Yield')
        pv01_val_res = thousand_dot_sep( pv01_val )
        
        pv01_1dc_val = abs( pv01_val - port_trades_calculation(cur_trades, calc_space, '1DayChange_PV01') )
        pv01_1dc_val_res = thousand_dot_sep( pv01_1dc_val )
        
        convex_val = port_trades_calculation(cur_instruments, calc_space, 'Portfolio Convex')
        convex_val_res = thousand_dot_sep( convex_val )
        
        convex_1dc_val = abs( convex_val - port_trades_calculation(cur_instruments, calc_space, '1DayChange_Convexity') )
        #convex_1dc_val = 0.0
        convex_1dc_val_res = thousand_dot_sep( convex_1dc_val )
        
        cs_val = port_trades_calculation(cur_instruments, calc_space, 'Flat Credit Spread Delta')
        cs_val_res = thousand_dot_sep( cs_val )
        
        cs_1dc_val = abs( cs_val - port_trades_calculation(cur_instruments, calc_space, '1DayChange_CS01') )

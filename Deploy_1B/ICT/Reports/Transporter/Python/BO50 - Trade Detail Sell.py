...
def get_val_query():
    ...
    last_week = f"date_add_delta('{today_date}', 0, 0, 7)"
...
def acm_get_val(ael_val, calc_space) :
    ...
    year, month, day = trade.AcquireDay().split("-")
    acquisition_date = f"{month}/{day}/{year}"
    
    AcquisitionPricePercent = calc_space.CalculateValue(trade, "AcquisitionPricePercent")
    AcquisitionAmount = calc_space.CalculateValue(trade, "Portfolio Realized Profit and Loss")
    AcquisitionAmount = "".join([x for x in str(AcquisitionAmount) if x.isdigit() or x == "."])
    
    list_row_val = [status, type, trdnbr, trade_date, settlement_date, br_code, im_code, security_code, b_or_s, price, face_value, trdnbr, type, trdnbr, face_value, 
                    acquisition_date, AcquisitionPricePercent, AcquisitionAmount]
...
def create_txt_file(query_val_list, report_folder, report_name):
    ...
    header_list_name = [
        "Transaction Status", "Data Type", "TD Reference ID", "Trade Date", "Settlement Date", "BR Code", "IM Code", "Security Code", "B/S", "Price", "Face Value", "TD Reference No", "Data Type",
        "TD Reference No", "Face Value", "Acquisition Date", "Acquisition Price (%)", "Acquisition Amount"
    ]
...
ael_gui_parameters={'runButtonLabel':'&&Run',
                    ...
                    'windowCaption':'BO50 - Trade Detail Sell'}
                    
ael_variables=[
['report_name','Report Name','string', None, 'BO50 - Trade Detail Sell', 1,0],

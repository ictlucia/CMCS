...
def balanceFormat(balanceRaw):
    ...
    position = "IN" if "C" in balanceUsed else "OUT" if "C" in balanceUsed else ""
    balanceUsed = float(balanceUsed.replace("D", "-").replace("C", ""))
        
    return [dateused, balanceUsed, position]

def getDataQuery():
    queryAcc = acm.FSQL["SGBO19 - Nostro Movement Realtime - Account"].Value().Text()
    querySett = acm.FSQL["SGBO19 - Nostro Movement Realtime - settlement"].Value().Text()
    ...
    for accData in accountList:
        ...
        print(accData)
        dateBalance, balanceNom, position = balanceFormat(accData[2])
        accData[1] = position
        accData[4] = dateBalance
        accData[2] = balanceNom
        
        data_dict[accData[0]] = {'' : [accData]}
    
    for settData in settList:
        if '' in data_dict[settData[0]].keys():
            data_dict[settData[0]][''].append(settData)
        else :
            data_dict[settData[0]][''] = [settData]
...
def openHtmlCode():
    ...
    <body>
        <table>
            <tr>
                <td colspan="4" style="border-top: dashed; width: 15cm; text-align: left">Branch : 02 Report : NOSMOVT</td>
                <td colspan="4" style="border-top: dashed; width: 15cm; text-align: center">NOSMOVT</td>
            </tr>
            <tr>
...
def headerCurrHtml(curr):
    html_code = f"""
        <tr><td></td></tr>
        <tr><td></td></tr>
        <tr><td colspan="8">SACCT: {curr}</td></tr>
    """
    return html_code
...
def headerField():
    html_code = f"""
        <tr>
            <td style="text-align:left; width:10cm">SACCT</td>
...
def dataBodyHtml(data_list):
    html_code = f"""
        <tr>
            ...
            <td style="text-align: center;">{data_list[1]}</td>
            <td style="text-align: right;">{f"{float(data_list[2]):,}"}</td>
            <td style="text-align: center;">{data_list[3]}</td>
            <td style="text-align: center;">{data_list[4]}</td>
            <td style="text-align: center;">{data_list[5]}</td>
            <td style="text-align: center;">{data_list[6]}</td>
            <td style="text-align: center;">{data_list[7]}</td>
            <td style="text-align: center;">{data_list[8]}</td>
        </tr>
    """
    return html_code
...
def totalAmountHtml(unit, value, title):
    htmlCode = f"""
        <tr><td colspan="2">Total{title} : {unit}</td></tr>
        <tr><td colspan="2"></td><td  style="text-align: right; border-top: solid;">{float(value):,}</td></tr>
    """
    return htmlCode
    ...
    for i, (curr, data_gl) in enumerate(getDataQuery().items()):
        ...
        for glNum, list_2d in data_gl.items():
            ...
            for j, data_list in enumerate(list_2d):
                htmlCode += headerField() if j == 0 else ""
                htmlCode += dataBodyHtml(data_list)
                valGl += float(data_list[2])
                
            valCurr += valGl
            #htmlCode += totalAmountHtml(glNum, valGl, "VDATE")
        htmlCode += totalAmountHtml(curr, valCurr, " SACCT")
...
ael_gui_parameters={'runButtonLabel':'&&Run',
                    ...
                    'windowCaption':'SGBO19 - Nostro Movement - Realtime'}
ael_variables=[
['report_name','Report Name','string', None, 'SGBO19 - Nostro Movement - Realtime', 1,0],

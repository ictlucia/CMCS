...
off_balance_sheet = {
    "FX Swap Transaction": prepare_query_data("('FX')", "('SWAP')")
    + [0 for _ in range(4)],
    "&emsp;Loan Commitment CIB005": [0 for _ in range(16)],
    "Loan Commitment EUR Helicopter": [0 for _ in range(16)],
    "&emsp;": ["" for _ in range(16)],
    "&emsp;": ["" for _ in range(16)],
    "&emsp;": ["" for _ in range(16)],
    "&emsp;": ["" for _ in range(16)],
    "&emsp;": ["" for _ in range(16)],
    "Total Off Balance sheet": [0 for _ in range(16)],
}
cash_inflow = {
...
contingency_plan = {
    "Asset Sale (Trade Finance/UPAS/TR Bank to Bank Financing)": [0 for _ in range(6)],
    "Inter Office Borrowing (USD 000)": [0 for _ in range(6)],
    "Interbank Borrowing (USD 000)": [0 for _ in range(6)],
    "HO Borrowing (USD 000)": [0 for _ in range(6)],
    "REPO (USD)": [0 for _ in range(6)],
    "Total Contingency Funding": [0 for _ in range(6)],
}
...
def prepare_header3(html_content, name):
    headers = [
        name,
        "&#8804; 7 days",
        "7 days-1 Mo",
    ...
    for i, header in enumerate(headers):
        if i == 0:
            ...
            html_content = html_gen.add_cell_data(
                html_content, header, "style = 'background-color : #b28dd6'"
            )
    html_content = html_gen.close_table_row(html_content)
    return html_content
...
def ael_main(parameter):
    ...
    # Assets
    html_content += '<table><tr><td colspan="2" style="font-weight : bold; background-color : #fcf2ae; text-align : left">Balance Sheet</td></tr></table>'
    ...
    # Total Assets Table
    total_assets_data = [["Total Assets", "", 0], ["Total Assets per BS", "", 0]]
    html_content += "<table><tr><td>&nbsp;</td></tr>"
    html_content = html_gen.add_data_row(
        html_content, total_assets_data, row_class="", cell_class=""
    )
    html_content += "</table>"
    html_content = html_gen.prepare_html_table(
        html_content,
    ...
    html_content = html_gen.add_data_row(
        ...
        aset_minus,
        row_class="",
        cell_class="style='background-color : #aec5eb'",
    )
    ...
    html_content = html_gen.add_data_row(
        html_content, total_liabilities_data, row_class="", cell_class=""
    )
    html_content = html_gen.prepare_html_table(
        html_content,
        [],
    ...
    html_content = prepare_table(html_content, off_balance_sheet, "Off Balance Sheet")
    ...
    html_content = prepare_header3(html_content, "Static Liquidity Gap")
    html_content = html_gen.add_data_row(
        html_content,
        static_liquidity_gap,
    ...
    html_content += "<tr><td>&nbsp;</td></tr>"
    html_content += '<tr><td colspan="2" style="font-weight : bold; text-align : left">Dynamic Gap</td></tr>'
    html_content = html_gen.prepare_html_table(
        html_content,
        [],
        row_style="",
    ...
    html_content = prepare_header3(html_content, "Dynamic Liquidity Gap")
    html_content = html_gen.add_data_row(
        html_content,
        dynamic_liquidity_gap,
        row_class="style='background-color : #aec5eb'",
    ...
    html_content += "<tr><td>&nbsp;</td></tr>"
    html_content += '<tr><td colspan="2" style="font-weight : bold; text-align : left">Contingency Plan</td></tr>'
    html_content = prepare_table(
        html_content,
        contingency_plan,
        "Funding Sources & Contingency Plan Scenarios",
        False,
        True,
    )
    contigency_liquidity_gap = [
        ["Periodic Contigency Gap", ""] + [0] * 6,
        ["Cummulative Contigency Gap", ""] + [0] * 6,
    ]
    html_content = html_gen.add_data_row(
        html_content,
        contigency_liquidity_gap,
        row_class="style='background-color : gray'",
        cell_class="",
    )

...
def query_process(optkey3="", optkey4="", portfolio=""):
    ...
    list_curr = ["USD", "RMB", "SGD", "EUR", "GBP", "JPY", "IDR"]
    usd_book = 0
...
port_list = [
    "FX Spot 1 BMSH - ACU",
    "FX Spot 2 BMSH - ACU",
    "FX Derivatives 1 BMSH - ACU",
    "FX Derivatives 2 BMSH - ACU",
    "IRT DCM 1 BMSH - ACU",
    "IRT DCM 2 BMSH - ACU",
    "IRT MM Depo Loan Repo RR 1 BMSH - ACU",
    "IRT MM Depo Loan Repo RR 2 BMSH - ACU",
    "IRT MM Bond RDPU 1 BMSH - ACU",
    "IRT MM Bond RDPU 2 BMSH - ACU",
    "IRT Derivative 1 BMSH - ACU",
    "IRT Derivative 2 BMSH - ACU",
    "BB Derivative BMSH - ACU",
    "BB FX BMSH - ACU",
    "BB BOND PL BMSH - ACU",
    "BB BOND AC BMSH - ACU",
    "BB BOND OCI BMSH - ACU",
    "BB BOND SHARIA PL BMSH - ACU",
    "BB BOND SHARIA AC BMSH - ACU",
    "BB BOND SHARIA OCI BMSH - ACU",
    "IBFI 1 BMSH - ACU",
    "CB 1 BMSH - ACU",
    "CB 2 BMSH - ACU",
    "CB 3 BMSH - ACU",
    ...
    "GVI 1 BMSH - ACU",
    "GVI 2 BMSH - ACU",
    "Commercial 1 BMSH - ACU",
    "Commercial 2 BMSH - ACU",
    "Commercial 3 BMSH - ACU",
    "Commercial 4 BMSH - ACU",
    "Commercial 5 BMSH - ACU",
    "Commercial 6 BMSH - ACU",
    "SAM 1 BMSH - ACU",
    "SAM 2 BMSH - ACU",
    "SAM 3 BMSH - ACU",
    "LIQ IB BMSH - ACU",
    "LIQ MM BMSH - ACU",
    "ALM Bilateral Loans BMSH - ACU",
    "ALM Bonds Issued BMSH - ACU",
    "LIQ NOS BMSH - ACU",
    "FX with Customer BMSH - ACU",
    "Bonds with Customer BMSH - ACU",
    "Derivative with Customer BMSH - ACU",
]
# port_list = "(" + ",".join([f"'{x}'" for x in port_list[:39]]) + ")"
query_result = [
    query_process("('DL')", "('CL', 'MD', 'CMP', 'CMT', 'OVP', 'BA')", port_list)
] * 5
query_result_total_1 = [0, 0, 0, 0, 0, 0, 0, 0]
...
content_a1 = {
    "INTER OFFICE ASSET-TRUST RECEIPT": ["GL 19611214/204 =< 1 MONTHS"]
    + query_result[0],
    "INTER OFFICE ASSET-UPAS": ["GL 19611205 =< 1 MONTHS"] + query_result[1],
    "BMSH Nostro at INTEROFFICE": ["Gap Change"] + query_result[2],
    "BMSH INTEROFFICE PLACEMENT =< 1 MONTHS": ["IP/CM"] + query_result[3],
...
query_result_total_2 = [0, 0, 0, 0, 0, 0, 0, 0]
for i in range(len(query_result[0])):
    list_row = [
        str(query_result[j][i]).replace(",", "") for j in range(len(query_result))
    ]
...
content_a2 = {
    "BMSH INTEROFFICE BORROWING =< 1 MONTHS": ["IT/CM"] + query_result[0],
...
sub_header_b = ["NETT INTEROFFICE POSSITIVE", ""] + [100] * 8
content_b = {
    "-1.1 CASH": [""] + query_result[0],
    "-1.2 GOLDEN": [""] + query_result[1],
    "-1.3 DEPOSIT WITH PBOC": [""] + query_result[2],
    "-1.4 DEPOSIT WITH INTERBANK": ["Time Deposit+Nostro+Over Amount of RD"]
    + query_result[3],
    "": ["Placement IP/FT"] + query_result[4],
    "-1.4 INTERBANK LENDING =< 1 MONTHS": ["Lending IP/CM"] + query_result[5],
    "-1.5 A/R INTEREST =< 1 MONTHS": ["Unch GA A Sum AccInt"] + query_result[6],
    ...
    "-1.6 LOANS =< 1 MONTHS": ["Ch GA Sum of Principle"] + query_result[11],
...
sub_header_c = ["NETT INTEROFFICE NEGATIVE", ""] + [100] * 8
content_c = {
...
total_summary_2 = {
    "TOTAL CURRENT ASSETS": ["88,225,761.27", "88,225,761.27", "88,225,761.27"],
    "TOTAL CURRENT LIABILITIES": ["107,719,801,65", "88,225,761.27", "88,225,761.27"],
}
#############################################################################
# GENERATE XLS TABLE
#############################################################################
...
def summary(html_code, total_summary):
    html_code += '<tr><td colspan="3" style="background-color: #3F95B8; color:white; font-weight:bold; text-align:center">SUMMARY USD BOOK</td></tr>'
...
def summary_2(html_code, total_summary):
    header_list = ["SUMMARY LIQUIDITY RATIO", "COMBINE", "USD", "RMB"]
    html_code += "<tr>"
    for header_val in header_list:
        html_code += f'<td style="background-color: #3F95B8; color:white; font-weight:bold; text-align:center">{header_val}</td>'
    html_code += "</tr>"
    for key, val_list in total_summary.items():
        html_code += f'<tr><td style="background-color: #ADD1DF">{key}</td>'
        for val in val_list:
            html_code += f'<td style="text-align: right">{val}</td>'
        html_code += "</tr>"
    val_summary_list_2d = [
        ["LIQUIDITY RATIO", "95%", "95%", "95%"],
        ["LIQUIDITY RATIO TRESHOLD", "above 25%", "above 25%", "above 25%"],
    ]
    for i, val_list in enumerate(val_summary_list_2d):
        html_code += "<tr>"
        for val in val_list:
...
def generate_excel(report_folder, report_name):
    ...
    html_code += '<tr><td style="text-align: center; font-weight:bold" colspan="10">LIQUIDITY RATIO ANALYSIS</td></tr>'
    html_code += f'<tr><td style="text-align: center; font-weight:bold" colspan="10">{date_today_ddmmyyyy}</td></tr>'
    html_code += "<tr><td></td></tr>"
    # html_code = currency_price(html_code, curr_price)
    html_code += '<tr><td style="text-align:left; font-weight:bold; width:380px">INTEROFFICE EXPOSURES</td></tr>'
    ...
    html_code += "<tr><td></td></tr>"
    html_code = summary_2(html_code, total_summary_2)
    red_text = ["=< 1 MONTHS", "=< 1 MONTH", "=< 1 MTH", "<= 1 M", "Over Amount of RD"]
...
ael_gui_parameters = {
    ...
    "windowCaption": "SH17a - Liquidity Ratio - USD Book",
}
ael_variables = [
    [
        "report_name",
        ...
        "string",
        None,
        "SH17a - Liquidity Ratio - USD Book",
        1,
        0,
    ],
    [

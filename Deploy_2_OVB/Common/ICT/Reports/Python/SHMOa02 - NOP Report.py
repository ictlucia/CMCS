...
ael_gui_parameters = {
    ...
    "windowCaption": "SHMOa02 - NOP Report",
}
ael_variables = [
    ["report_name", "Report Name", "string", None, "SHMOa02 - NOP Report", 1, 0],
    [
        "file_path",
        "Folder Path",
...
def ael_main(parameter):
    ...
    html_content = html_gen.create_base_html_content(
        "NOP Report - " + current_date, title_style
    )
    title = [
        "DESKRIPSI SANDI",
        "VALUTA",
    ...
    html_content = html_gen.add_data_row(
        html_content, [non_idr_ccy], "style=font-weight:bold"
    )
    html_content = html_gen.add_data_row(html_content, [idr_ccy])
    html_content = html_gen.add_data_row(
        html_content, [bmhk_nop], "style=font-weight:bold"
    )
    for title_blue in title:
        html_content = html_content.replace(
            "<th>" + title_blue + "</th>",
            "<th style='background-color:blue;color:white'>" + title_blue + "</th>",
        )

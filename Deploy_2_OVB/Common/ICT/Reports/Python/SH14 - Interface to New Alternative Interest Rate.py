...
ael_variables = [
    [
        ...
        "string",
        None,
        "SH14 - Interface to New Alternative Interest Rate",
        1,
        0,
    ],
    ...
    [
        ...
        "Market Type",
        "string",
        ["LIBOR", "CIBOR", "JIBOR", "SBI1M"],
        "LIBOR",
        1,
        0,
        "Select to Filter the Market",
        None,
        1,
    ],
    [
...
def ael_main(parameter):
    ...
    input_date = "-".join(
        [str(i).zfill(2) for i in acm.Time.DateToYMD(parameter["input_date"])]
    )
    ...
    columns = [
        ...
        "RESULT",
        "PREVIOUS DAY RATE",
        "FAIRNESS (COMPARE WITH PREVIOUS DAY RATE)",
    ]
    rows = [["BID", "ASK", "MID"] * 5]
    for tenor in tenors:
        temp_row = [tenor]
    ...
    for col in columns:
        if col.lower() == "tenor":
            table_html = table_html.replace(
                "<th>{}</th>".format(col),
                '<th rowspan=2 style="background-color:#FFFFFF;font-weight:bold">{}</th>'.format(
                    col
                ),
            )
        else:
            table_html = table_html.replace(
                "<th>{}</th>".format(col),
                '<th colspan=3 style="background-color:#FFFFFF;font-weight:bold">{}</th>'.format(
                    col
                ),
            )
    for tenor in tenors:
        table_html = table_html.replace(
            "<td>{}</td>".format(tenor),
            '<td style="background-color:#FFFFFF;font-weight:bold">{}</td>'.format(
                tenor
            ),
        )
    for k in set(rows[0]):
        table_html = table_html.replace(
            "<td>{}</td>".format(k),
            '<td style="background-color:#FFFFFF;font-weight:bold">{}</td>'.format(k),
        )
    table_html = table_html.replace(
        "<td>NOK</td>",
        '<td style="background-color:#F90000;font-weight:normal">NOK</td>',
    )
    for row in rows[1:]:
        ...
        for value in fairness:
            if float(value) > 1 or float(value) < -1:
                new_text = new_text.replace(
                    "<td>{}</td>".format(value),
                    '<td style="background-color:#F90000;font-weight:normal">{}</td>'.format(
                        value
                    ),
                )

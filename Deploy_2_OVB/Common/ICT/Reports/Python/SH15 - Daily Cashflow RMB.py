...
def get_data(
    ...
    if (
        ...
        elif time_bucket == "2 days - 7 days":
            time_query = (
                " and "
                + end_date
                + " >'"
        ...
        for row in result[1][0]:
            ...
            if any(col != None for col in row):
                if tradeCurr == "CNY":
                    ...
                    usd_price = usd_price_mtm(acm.FTrade[row[2]], "CNY")
                    nominal = (
...
ael_gui_parameters = {
    ...
    "windowCaption": "SH15 - Daily Cashflow RMB",
}
ael_variables = [
    ["report_name", "Report Name", "string", None, "SH15 - Daily Cashflow RMB", 1, 0],
    [
        "file_path",
        "Folder Path",
    ...
    ["currency", "Currency Of Choice", "string", ["CNY"], "CNY", 0, 0],
    [
        "output_file",
...
def ael_main(parameter):
    ...
    html_content = html_gen.open_table_row(html_content)
    html_content = html_gen.add_cell_data(
        html_content, "Dlm CNY (Million)", 'colspan="3" class="subtitle-3"'
    )
    html_content = html_gen.close_table_row(html_content)

...
dict_of_column_filter = {
    ...
    "LF": {"Product Type": ["DL"], "Category": ["LF"]},
    "DF": {"Product Type": ["DL"], "Category": ["FASBI"]},
    "Term Deposit": {"Product Type": ["TD"], "Category": ["BI"]},
    "Repo BI": {
        "Product Type": ["REPO"],
    ...
    "Coupon SB Terbit": {"Product Type": ["DEBT"], "Category": ["BOND", "ZCO", "NCD"]},
    "Reksadana": {"Product Type": ["BOND"], "Category": ["RDPU", "RDPT"]},
    "CCS": {"Product Type": ["SWAP"], "Category": ["CCS"]},
    "IRS": {"Product Type": ["SWAP"], "Category": ["IRS"]},
    "SWAP": {"Product Type": ["FX"], "Category": ["SWAP"]},
    "Term Deposit BI": {"Product Type": ["TD"], "Category": ["BI"]},
    "SBBI Valas": {"Product Type": ["SBI"], "Category": ["IDSV"]},
}
    ...
    try:
        ...
        for cf in cashflows:
            ...
            if cashflow_type == "Fixed Rate" or cashflow_type == "Fixed Amount":
                total += get_projected_from_money_flow(
                    trade, cf.PayDate(), cashflow_type
                )
...
def get_date_from_cashflow_for_proj_value(trade, date_input):
    ...
    try:
        ...
        for cf in cashflows:
            ...
            cashflow_type = cf.CashFlowType()
            if time_range == 0 and (
                cashflow_type == "Fixed Rate" or cashflow_type == "Fixed Amount"
            ):
...
def get_nominal_from_trade(
    valueday, dict_of_column_filter, filter_currency, column_name, title_type
):
    ...
    for i in all_trades[column_name]:
        ...
        try:
            ...
            list_column_ins_date = [
                "Repo BI",
                "Reverse Repo BI",
                "Repo Interbank",
                "Reverse Repo Interbank",
            ]
            list_column_cashflow_date = [
                "Interbank Placement",
                "Interbank Taken",
                "LF",
                "DF",
            ...
            if column_name in list_column_ins_date and title_type == "Maturity":
                ...
                time_range = acm.Time.DateDifference(
                    valueday, i.Instrument().StartDate()
                )
            elif column_name in list_column_cashflow_date:
                # Kalo Tanggal di baris tabel == Pay Date nya di Cashflow
                time_range, cashflow_type = get_date_from_cashflow_for_proj_value(
                    i, valueday
                )
            else:
            ...
            if dict_of_column_filter["Product Type"] != []:
                try:
                    status_opt_key_3 = bool(
                        i.OptKey3().Name() in dict_of_column_filter["Product Type"]
                    )
                except:
            ...
            list_else = (
                list_column_ins_date
                + list_column_trd_time
                + list_column_cashflow_date
                + ["SWAP"]
            )
            if (
                (filter_currency == temp_curr or filter_currency == temp_instrument)
                and temp_category in list_category
                and time_range == 0
                and status_opt_key_3
            ):
                if column_name not in list_else:
                    ...
                    title_type == "Maturity"
                    and column_name == "DF"
                    and str(i.InstrumentSubType()) == "Deposit/Loan"
                    and i.Premium() >= 0
                    and temp_curr != "IDR"
                    and filter_currency != "IDR"
                ):  # Tabel Kiri, DF
                    total_result += get_projected_from_money_flow(
                        i, valueday, cashflow_type
                    )
                    status_empty_transaction = False
                elif (
                    title_type == "Over"
                    ...
                    title_type == "Maturity"
                    and column_name == "Interbank Placement"
                    and str(i.InstrumentSubType()) == "Deposit/Loan"
                    and i.Premium() >= 0
                ):  # Tabel Kiri, Interbank Placement
                    total_result += get_projected_from_money_flow(
                        i, valueday, cashflow_type
                    )
                    status_empty_transaction = False
                    ...
                    and str(i.InstrumentSubType()) == "Deposit/Loan"
                    and i.Premium() >= 0
                ):  # Tabel Kanan, Interbank Placement
                    total_result += i.Premium()
                    status_empty_transaction = False
                elif (
                    title_type == "Maturity"
                    and column_name == "Interbank Taken"
                    and str(i.InstrumentSubType()) == "Deposit/Loan"
                    and i.Premium() < 0
                ):  # Tabel Kiri, Interbank Taken
                    total_result += get_projected_from_money_flow(
                        i, valueday, cashflow_type
                    )
                    status_empty_transaction = False
                    ...
                    and str(i.InstrumentSubType()) == "Deposit/Loan"
                    and i.Premium() < 0
                ):  # Tabel Kanan, Interbank Taken
                    total_result += i.Premium()
                    status_empty_transaction = False
                elif (
                    title_type == "Over"
                    and column_name == "SWAP"
                    and str(i.InstrumentSubType()) == "FX Cash"
                    and i.TradeProcessesToString() == "Swap Near Leg"
                ):  # Tabel Kanan, SWAP
                    ...
                    and column_name == "SWAP"
                    and str(i.InstrumentSubType()) == "FX Cash"
                    and i.TradeProcessesToString() == "Swap Far Leg"
                ):  # Tabel Kiri, SWAP
                    total_result += get_value_for_swap_trade(i, filter_currency)
                    status_empty_transaction = False
                elif (
                    title_type == "Maturity"
                    and column_name == "Repo BI"
                    and "Repo/Reverse" in str(i.InstrumentSubType())
                    and i.StartCash() < 0
                ):  # Tabel Kiri, Repo BI
                    ...
                    title_type == "Maturity"
                    and column_name == "Reverse Repo BI"
                    and "Repo/Reverse" in str(i.InstrumentSubType())
                    and i.StartCash() >= 0
                ):  # Tabel Kiri, Reverse Repo BI
                    total_result += get_sum_cashflow_from_trade(i)
                    status_empty_transaction = False
                elif (
                    title_type == "Over"
                    ...
                    and "Repo/Reverse" in str(i.InstrumentSubType())
                    and i.StartCash() >= 0
                ):  # Tabel Kanan, Reverse Repo BI
                    total_result += i.StartCash()
                    status_empty_transaction = False
                elif (
                    title_type == "Maturity"
                    and column_name == "Repo Interbank"
                    and "Repo/Reverse" in str(i.InstrumentSubType())
                    and i.StartCash() < 0
                ):  # Tabel Kiri, Repo Interbank
                    total_result += get_sum_cashflow_from_trade(i)
                    status_empty_transaction = False
                elif (
                    title_type == "Over"
                    ...
                    and "Repo/Reverse" in str(i.InstrumentSubType())
                    and i.StartCash() < 0
                ):  # Tabel Kanan, Repo Interbank
                    total_result += i.StartCash()
                    status_empty_transaction = False
                elif (
                    title_type == "Maturity"
                    and column_name == "Reverse Repo Interbank"
                    and "Repo/Reverse" in str(i.InstrumentSubType())
                    and i.StartCash() >= 0
                ):  # Tabel Kiri, Reverse Repo Interbank
                    total_result += get_sum_cashflow_from_trade(i)
                    status_empty_transaction = False
                elif (
                    title_type == "Over"
                    ...
                    and "Repo/Reverse" in str(i.InstrumentSubType())
                    and i.StartCash() >= 0
                ):  # Tabel Kanan, Reverse Repo Interbank
                    total_result += i.StartCash()
                    status_empty_transaction = False
                elif (
                    title_type == "Maturity"
                    and column_name == "Term Deposit"
                    and i.Premium() < 0
                ):  # Hanya Tabel Kiri, Term Deposit
                    total_result += get_projected_from_money_flow(
                        i, valueday, cashflow_type
                    )
                    status_empty_transaction = False
                    ...
                    title_type == "Maturity"
                    and column_name == "Surat Berharga Dimiliki"
                    and trd_time_difference <= 3
                ):  # Tabel Kiri, Surat Berharga Dimiliki
                    total_result += i.Premium()
                    status_empty_transaction = False
                elif (
                    title_type == "Over"
                    and column_name == "Surat Berharga Dimiliki"
                    and trd_time_difference > 3
                ):  # Tabel Kanan, Surat Berharga Dimiliki
                    total_result += i.Premium()
                    status_empty_transaction = False
                elif (
                    title_type == "Maturity"
                    and column_name == "Surat Berharga Terbit"
                    and trd_time_difference <= 3
                ):  # Tabel Kiri, Surat Berharga Terbit
                    total_result += i.Premium()
                    status_empty_transaction = False
                elif (
                    title_type == "Over"
                    and column_name == "Surat Berharga Terbit"
                    and trd_time_difference > 3
                ):  # Tabel Kanan, Surat Berharga Terbit
                    total_result += i.Premium()
                    status_empty_transaction = False
                elif (
                    title_type == "Maturity"
                    ...
                    title_type == "Maturity" and column_name == "Coupon SB Dimiliki"
                ):  # Tabel Kiri, Coupon SB Dimiliki (Hanya Maturity)
                    total_result += get_projected_from_money_flow(
                        i, valueday, cashflow_type
                    )
                    status_empty_transaction = False
                elif (
                    title_type == "Maturity" and column_name == "Coupon SB Terbit"
                ):  # Tabel Kiri, Coupon SB Terbit (Hanya Maturity)
                    total_result += get_projected_from_money_flow(
                        i, valueday, cashflow_type
                    )
                    status_empty_transaction = False
...
ael_gui_parameters = {
    ...
    "windowCaption": "SGBO35 - Cashflow Report for All Products",
}
...
ael_variables = [
    ...
    [
        ...
        "From (Start Date)",
        "string",
        [acm.Time.DateToday(), "1M", "6M", "1Y", next_month, next_year],
        acm.Time.DateToday(),
        1,
        0,
    ],
...
def ael_main(parameter):
    ...
    time_range = acm.Time.DateDifference(end_date, selected_date) + 1
    count = -1
    global all_trades
    ...
    for ins in dict_of_column_filter.keys():
        ...
        query = (
            """
        select t.trdnbr, t.status, c3.entry 'ProductType', c4.entry 'Category', i.insid 'Currency'
        from trade t, choicelist c3, choicelist c4, instrument i
        where (t.optkey3_chlnbr=c3.seqnbr) and (t.optkey4_chlnbr=c4.seqnbr) and (t.curr=i.insaddr)
    ...
    for i in range(time_range):
        temp_date = acm.Time.DateAddDelta(selected_date, 0, 0, i)
        check_holiday = (
            acm.FCalendar["No Holiday"].CalendarInformation().IsNonBankingDay(temp_date)
        )  # Cek apakah hari libur/bukan
        if check_holiday:
            continue
        list_date.append(temp_date)
        (
            temp_row_ins_maturity,
            temp_row_ins_over,
            temp_row_sub_total_maturity,
            temp_row_sub_total_over,
        ) = ([], [], 0, 0)
        for maturity in title_ins_maturity:
            ...
            if maturity in list(dict_of_column_filter.keys()):
                temp_maturity_value = get_nominal_from_trade(
                    temp_date,
                    dict_of_column_filter[maturity],
                    filter_currency,
                    maturity,
                    title_type,
                )
        ...
        for over in title_ins_over:
            ...
            if over in list(dict_of_column_filter.keys()):
                temp_over_value = get_nominal_from_trade(
                    temp_date,
                    dict_of_column_filter[over],
                    filter_currency,
                    over,
                    title_type,
                )
        ...
        try:
            if filter_currency == "IDR":
                row_sub_total_maturity.append(
                    round_half_up(temp_row_sub_total_maturity, 3)
                )
            else:
                row_sub_total_maturity.append(
                    round_half_up(temp_row_sub_total_maturity, 2)
                )
        except:
    ...
    for i in list_date:
        temp_row = [
            [i]
            + row_ins_maturity[count]
            + [row_sub_total_maturity[count]]
            + row_ins_over[count]
            + [row_sub_total_over[count]]
            + [row_total[count]]
            + [row_cummulative[count]]
        ]
        final_row = []
    ...
    for i in titles:
        if "Instrument" in i:
            ...
            table_html = table_html.replace(
                "<th>" + i + "</th>",
                '<th colspan="' + column_length + '">' + i + "</th>",
            )
        else:
            table_html = table_html.replace(
                "<th>" + i + "</th>", '<th rowspan="2">' + i + "</th>"
            )
    for maturity in title_ins_maturity:
        table_html = table_html.replace(
            "<td>" + maturity + "</td>", "<td><b>" + maturity + "</b></td>"
        )
        table_xsl_fo = table_xsl_fo.replace(
            '<fo:table-cell border-width="1px" border-style="solid" padding="8pt"><fo:block>'
            + maturity
            + "</fo:block></fo:table-cell>",
            '<fo:table-cell border-width="1px" border-style="solid" padding="8pt" font-weight="bold" text-align="center"><fo:block>'
            + maturity
            + "</fo:block></fo:table-cell>",
        )
    for over in title_ins_over:
        table_html = table_html.replace(
            "<td>" + over + "</td>", "<td><b>" + over + "</b></td>"
        )
        table_xsl_fo = table_xsl_fo.replace(
            '<fo:table-cell border-width="1px" border-style="solid" padding="8pt"><fo:block>'
            + over
    ...
    <fo:table-body>""",
        '''    <fo:table-body>
            ...
            <fo:table-cell padding="8pt" border-width="1px" font-weight="bold" text-align="center" border-style="solid" number-columns-spanned="'''
        + str(dict_len_title["Maturity"])
        + '''"><fo:block>Instrument (Maturity)</fo:block></fo:table-cell>
            ...
            <fo:table-cell padding="8pt" border-width="1px" font-weight="bold" text-align="center" border-style="solid" number-columns-spanned="'''
        + str(dict_len_title["Over"])
        + """"><fo:block>Instrument (over Tom/Spot)</fo:block></fo:table-cell>
            <fo:table-cell padding="8pt" border-width="1px" font-weight="bold" text-align="center" border-style="solid" number-rows-spanned="2"><fo:block>Sub Total</fo:block></fo:table-cell>
    ...
    print(
        f"{filter_currency} Projections, It took {end_time- start_time: 0.2f} second(s) to complete."
    )

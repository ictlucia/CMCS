...
all_trades = [trd for trd in acm.FTrade.Select('') if trd.OptKey3() is not None and trd.OptKey4() is not None]
...
def get_trades_by_prod_type_category(trade_list, ins_type):
    filtered_trades = []
    if dict_of_trade_filter[ins_type]['Product Type PosNeg Flag'] == 'Pos' and dict_of_trade_filter[ins_type]['Category PosNeg Flag'] == 'Pos':
        for trade in trade_list:
            if trade.OptKey3().Name() in dict_of_trade_filter[ins_type]['Product Type'] and trade.OptKey4().Name() in dict_of_trade_filter[ins_type]['Category']:
                filtered_trades.append( (trade, 'Trade') )
    elif dict_of_trade_filter[ins_type]['Product Type PosNeg Flag'] == 'Pos' and dict_of_trade_filter[ins_type]['Category PosNeg Flag'] == 'Neg':
        for trade in trade_list:
            if trade.OptKey3().Name() in dict_of_trade_filter[ins_type]['Product Type'] and trade.OptKey4().Name() not in dict_of_trade_filter[ins_type]['Category']:
                filtered_trades.append( (trade, 'Trade') )
    elif dict_of_trade_filter[ins_type]['Product Type PosNeg Flag'] == 'Neg' and dict_of_trade_filter[ins_type]['Category PosNeg Flag'] == 'Pos':
        for trade in trade_list:
            if trade.OptKey3().Name() not in dict_of_trade_filter[ins_type]['Product Type'] and trade.OptKey4().Name() in dict_of_trade_filter[ins_type]['Category']:
                filtered_trades.append( (trade, 'Trade') )
    elif dict_of_trade_filter[ins_type]['Product Type PosNeg Flag'] == 'Neg' and dict_of_trade_filter[ins_type]['Category PosNeg Flag'] == 'Neg':
        for trade in trade_list:
            if trade.OptKey3().Name() not in dict_of_trade_filter[ins_type]['Product Type'] and trade.OptKey4().Name() not in dict_of_trade_filter[ins_type]['Category']:
                filtered_trades.append( (trade, 'Trade') )
    return filtered_trades



def get_trades_by_cpty_ins_type(country, ins_type_list):

    pty_list = get_party_data_by_ael_sql(country)
    trade_list = [ (pty_list[0], 'Country' ) ]
    if len(pty_list) > 0:
...
def get_countries_by_ael_sql(country_list):
    '''
    country_list = []
    for pty in country_list:
        #print(pty[0])
        country = acm.FChoiceList[ pty[1] ]
        country_list.append(country)
    '''
    country_list = acm.FChoiceList.Select("list='Country of Risk'")[1:]
    return country_list
...
def get_party_data_by_ael_sql(country):
    ...
    pty_list = acm.FParty.Select("riskCountry='"+country.Name()+"' and type='Counterparty'")
    if pty_list.Size() > 0:
...
def get_journal_from_trade(trade):
    ...
    try:
        ...
        journals = acm.FJournal.Select("journalInformation='"+str(journal_info.Oid())+"'")
...
ael_gui_parameters={'runButtonLabel':'&&Run',
                    ...
                    'windowCaption':'SGBO22 - Consolidated Country Limit - Ultimate'}

#settings.FILE_EXTENSIONS

ael_variables=[
['report_name','Report Name','string', None, 'SGBO22 - Consolidated Country Limit - Ultimate', 1,0],
...
def ael_main(parameter):
    ...
    for i in country_list:
        ...
        if i is not None:
            ...
            all_ports = all_ports + get_trades_by_cpty_ins_type(i, dict_of_trade_filter.keys())
    ...
    titles = ['COUNTRY NAME', 'CURTYPE', 'COUNTRY LIMIT', 'EXP.DATE', 'BRANCH', 'SOURCE', 'SUBLMTNAME', 'TYPE UTILIZATION', 'COUNTERPARTY NAME', 'ACCTNO', 'ORICURRENCY', 'LIMIT DUE DATE', 'TOTAL']
    ...
    calc_space = acm.Calculations().CreateCalculationSpace(acm.GetDefaultContext(), 'FJournalSheet')
    ...
    for port in all_ports:
        ...
        if port[1] == 'Country':
            ...
            try:
                ...
                temp_row.append( big_decimal_number( get_threshold_limit_by_rule_name2(rule_name) ) )
            ...
            temp_row.append( get_rule_type_by_rule_name(rule_name) )
            ...
            try:
                ...
                temp_row.append( date_formatter( get_end_date_by_rule_name(obj, rule_name) ) )
            ...
            temp_row.append( get_exp_date_from_trade(obj_trade) )

            temp_row.append( get_branch_from_trade(obj_trade) )

            temp_row.append( get_source_from_trade(obj_trade) )
            ...
            try:
                ...
                temp_row.append( get_account_number_from_trade(journal, calc_space, 'Journal Account Number') )
    ...
    for i in titles:
        ...
        table_html = table_html.replace('<th>'+i+'</th>', '<th style="background-color: #000000; color: #ffffff;">'+i+'</th>')

    

    for cyan in cyan_rows:

        table_html = table_html.replace('<td>'+cyan+'</td>', '<td style="text-align: left; background-color: #00ccff;">'+cyan+'</td>') 

        table_xsl_fo = table_xsl_fo.replace('<fo:table-cell border-width="1px" border-style="solid" padding="8pt"><fo:block>'+cyan+'</fo:block></fo:table-cell>',

        '<fo:table-cell border-width="1px" border-style="solid" padding="8pt" text-align="left" background-color="#00ccff"><fo:block>'+cyan+'</fo:block></fo:table-cell>')

    
    for blue in blue_rows:

        table_html = table_html.replace('<tr><td>'+blue+'</td>', '<tr style="text-align: left; background-color: #0033cc;"><td>'+blue+'</td>')

        table_xsl_fo = table_xsl_fo.replace('''
        <fo:table-row>
        <fo:table-cell border-width="1px" border-style="solid" padding="8pt"><fo:block>{}</fo:block></fo:table-cell>'''.format(blue),
        '''
        <fo:table-row background-color="#0033cc">
        <fo:table-cell border-width="1px" border-style="solid" padding="8pt" text-align="left"><fo:block>{}</fo:block></fo:table-cell>'''.format(blue)
        )
    ...
    <fo:table-cell padding="8pt" border-width="1px" border-style="solid"><fo:block>Country Name</fo:block></fo:table-cell>
        <fo:table-cell padding="8pt" border-width="1px" border-style="solid"><fo:block>CurTyp</fo:block></fo:table-cell>
        <fo:table-cell padding="8pt" border-width="1px" border-style="solid"><fo:block>Country Limit</fo:block></fo:table-cell>
        <fo:table-cell padding="8pt" border-width="1px" border-style="solid"><fo:block>Exp. Date</fo:block></fo:table-cell>
        <fo:table-cell padding="8pt" border-width="1px" border-style="solid"><fo:block>Branch</fo:block></fo:table-cell>
        ...
        <fo:table-cell padding="8pt" border-width="1px" border-style="solid"><fo:block>Limit Due Date</fo:block></fo:table-cell>
    ...
    <fo:table-row background-color="#000000" color="#ffffff" font-weight="bold">
        ...
        <fo:table-cell padding="8pt" border-width="1px" border-style="solid"><fo:block>Currency</fo:block></fo:table-cell>
        <fo:table-cell padding="8pt" border-width="1px" border-style="solid"><fo:block>Customer Limit</fo:block></fo:table-cell>
        <fo:table-cell padding="8pt" border-width="1px" border-style="solid"><fo:block>Exp. Date</fo:block></fo:table-cell>
        <fo:table-cell padding="8pt" border-width="1px" border-style="solid"><fo:block>Branch</fo:block></fo:table-cell>
        <fo:table-cell padding="8pt" border-width="1px" border-style="solid"><fo:block>Source</fo:block></fo:table-cell>
        <fo:table-cell padding="8pt" border-width="1px" border-style="solid"><fo:block>SubLmtName</fo:block></fo:table-cell>

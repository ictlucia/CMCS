...
port_list = port_list_acu + port_list_dbu
query_result = [
...
content_a1 = {
    "INTER OFFICE ASSET-TRUST RECEIPT": ["GL 19611214"] + query_result[0],
    "INTER OFFICE ASSET-UPAS": ["GL 19611205"] + query_result[1],
...
header_a2 = ["INTEROFFICE PLACEMENT TO BMSH", "", "(TOTAL RMB)", "RMB BOOK", "USD BOOK"]
content_a2 = {
...
for i in range(len(query_result[0])):
    ...
    query_result_total_1[i] += sum([float(i) for i in list_row])
header_b = ["1. CURRENT ASSETS", "", "TOTAL (USD)", "RMB", "USD"]
...
content_b = {
    ...
    "-1.4 DEPOSIT WITH INTERBANK": [""] + query_result[3],
    "-1.4 INTERBANK PLCMT =< 1 MONTHS": [""] + query_result[4],
    "-1.4 INTERBANK LENDING =< 1 MONTHS": [""] + query_result[5],
...
for i in range(len(query_result[0])):
    ...
    query_result_total_2[i] += sum([float(i) for i in list_row])
header_c = ["2. CURRENT LIABILITIES", "", "TOTAL (USD)", "RMB", "USD"]
sub_header_c = ["NETT INTEROFFICE NEGATIVE", ""] + [""] * 3
content_c = {
    "-2.1 CHECKING ACCOUNT": [""] + query_result[0],
    "-2.2 DEPOSIT <= 1M": [""] + query_result[1],
    "-2.3 INTERBANK DEPO": [""] + query_result[2],
    "-2.3 INTERBANK TAKEN =< 1 MONTH": [""] + query_result[3],
    ...
    "-2.7 OTHER BOROWG =< 1 MONTH": [""] + query_result[9],
...
def currency_price(html_code, curr_price):
    ...
    html_code += f'<tr><td rowspan="6" style="border-right: 1px black solid; font-weight: bold; text-align:center; vertical-align:top">30-sep-22</td>'
    html_code += f'<td colspan="2" style="border-top: 1px black solid; font-weight: bold; background-color:#F8B4FF">{list(curr_price.keys())[0]}</td><td style="border-top: 1px black solid; border-right: 1px black solid; font-weight: bold; background-color:#F8B4FF">{list(curr_price.values())[0]}</td></tr>'
    for key, val in curr_price.items():
        if key != "USD/CNY":
            html_code += f'<tr><td colspan="2" style="font-weight: bold; background-color:#F8B4FF">{key}</td><td style="border-right: 1px black solid; font-weight: bold; background-color:#F8B4FF">{val}</td></tr>'
    return html_code
...
def summary(html_code, total_summary):
    border_full = "border: 1px solid black;"
    html_code += f'<tr><td colspan="3" style="background-color: yellow; color:red; {border_full} font-weight:bold; text-align:center">COMBINE IN RMB</td></tr>'
...
def generate_excel(report_folder, report_name):
    ...
    # html_code += '<tr><td style="text-align: center; font-weight:bold" colspan="5">LIQUIDITY RATIO ANALYSIS - COMBINE IN RMB</td></tr>'
    ...
    red_text = ["=< 1 MONTHS", "=< 1 MONTH", "=< 1 MTH", "<= 1 M"]
...
ael_gui_parameters = {
    ...
    "windowCaption": "SH17c - Liquidity Ratio - Combine in RMB",
}
ael_variables = [
    [
        "report_name",
        ...
        "string",
        None,
        "SH17c - Liquidity Ratio - Combine in RMB",
        1,
        0,
    ],
    [

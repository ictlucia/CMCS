...
['currencyTo', 'Currency To', 'string', ['SGD', 'USD', 'IDR'], 'SGD', 1, 0],
...
def ael_main(parameter):
    ...
    currencyTo = str(parameter['currencyTo'])
    ...
    for ccy in ccys:
        ...
        queryToday = f"instrument IN ('{ccy}', '{currencyTo}') AND currency IN ('{ccy}', '{currencyTo}') AND historical = true AND market = 'EOD_MtM' and day = TODAY"
        
        price_list_fa = acm.FPrice.Select(queryToday)
        ...
        if len(price_list_fa) > 0 :
            ...
            if price_fa.Instrument().Name() == currencyTo:
            
                bid_fa, ask_fa, ntr_fa = [1/bid_fa, 1/ask_fa, 1/ntr_fa] 
                    
            queryYesterday = f"instrument IN ('{ccy}', '{currencyTo}') AND currency IN ('{ccy}', '{currencyTo}') AND historical = true AND market = 'EOD_MtM' and day = TODAY"
        
            price_list_fa_yesterday = acm.FPrice.Select(queryYesterday)
            ...
            if len(price_list_fa_yesterday) > 0 :
                ...
                if price_fa_ytd.Instrument().Name() == currencyTo:
            
                    ntr_fa_yesterday = 1/ntr_fa_yesterday 

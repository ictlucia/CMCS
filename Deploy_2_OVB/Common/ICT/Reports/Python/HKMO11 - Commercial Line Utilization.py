...
def compliance_rule_list(rule_def, rule_category):
    return [
        comp_rule.Name()
        for comp_rule in acm.FComplianceRule.Select(
            "definitionInfo='" + rule_def + "' and ruleCategory='" + rule_category + "'"
        )
    ]
...
ael_gui_parameters = {
    ...
    "windowCaption": "HKMO11 - Commercial Line Utilization",
}
...
ael_variables = [
    [
        ...
        "string",
        None,
        "HKMO11 - Commercial Line Utilization",
        1,
        0,
    ],
    [
    ...
    [
        ...
        "Compliance Rule",
        "string",
        compliance_rule_list("PositionAndRiskControl", "Pre-Deal"),
        "TQA CEM LIMIT INCREASE",
        0,
        0,
        "Select Compliance rule Name",
    ],
]
...
def ael_main(parameter):
    ...
    rulename = str(parameter["compliance_rule_name"])
    ...
    # party_list = ['Counterparty 1', 'Counterparty 5']
    ...
    for i in party_list:
        pty = i[0]
        all_ports.append((pty, i[1], i[2], i[3], i[4], i[5], i[6], i[7], i[8], i[9]))
    ...
    total_columns = ["Comm Line (Rp)", "Total Utilisasi (Rp)"]
    total2_columns = ["Credit Line (Rp)", "Utilisasi (Rp)"]
    columns = country_columns + total_columns + total2_columns
    ...
    for port in all_ports:
        ...
        temp_row.append(get_threshold_limit_by_rule_name(obj, rulename))
        temp_row.append(get_utilization_val_by_rule_name(obj, rulename))
        temp_row.append(get_threshold_limit_by_rule_name(obj, rulename))
        temp_row.append(port[1])
        temp_row.append(port[2])
        temp_row.append(port[3])
        temp_row.append(port[4])
        temp_row.append(port[5])
        temp_row.append(port[6])
        temp_row.append(port[7])
        temp_row.append(port[8])
        temp_row.append(port[9])
        try:
            temp_row.append(date_formatter(get_end_date_by_rule_name(obj, rulename)))
        except:
            temp_row.append("-")
        if len(temp_row) > 1:
    ...
    # current_date2 = get_current_report_datetime("-")
    current_date2 = (
        "Report Date: "
    ...
    <fo:table-cell padding="8pt" border-width="1px" border-style="solid"><fo:block>No</fo:block></fo:table-cell>
        ...
        <fo:table-cell padding="8pt" border-width="1px" border-style="solid"><fo:block>Total</fo:block></fo:table-cell>
        <fo:table-cell padding="8pt" border-width="1px" border-style="solid"><fo:block>Maturity</fo:block></fo:table-cell>
        </fo:table-header>
    <fo:table-body>""",
        '''<fo:table-body>
    <fo:table-row background-color="#000000" color="#ffffff" font-weight="bold">
        ...
        <fo:table-cell padding="8pt" border-width="1px" border-style="solid" number-columns-spanned="'''
        + str(len(total2_columns))
        + """"><fo:block>Total</fo:block></fo:table-cell>
        <fo:table-cell padding="8pt" border-width="1px" border-style="solid" number-rows-spanned="2"><fo:block>Maturity</fo:block></fo:table-cell>
    </fo:table-row>""",
    )

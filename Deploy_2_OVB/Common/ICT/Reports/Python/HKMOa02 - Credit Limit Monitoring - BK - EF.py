        ...
        for appliedRule in [ars for ars in cr.AppliedRules() if ars.Inactive() == False]:
            ...
            if dateToday < endDate:
                ...
                if targetType == 'ChoiceList':
                    ...
                    desc = target.AssignInfo()
                    idcust = target.Oid()
                elif targetType == 'Party':
                    desc = target.Fullname()
                    idcust = target.Id2()
                elif targetType == 'HierarchyNode':
                    desc = target.DisplayName()
                    idcust = target.Oid()
                else:
                    desc, idcust = "-", "-"
                ...
                if (desc is None) or (desc.strip() == ""):
                    desc = "-"
                if (idcust is None) or (idcust.strip() == ""):
                    idcust = "-"
                ...
                ### COMBINE ###
                content_list.append([str(desc), str(idcust), str(ccy), str(limAmt), str(expDate), str(compliance_rule_name), str(limAmt_usd), str(exposure_usd), str(term)])
    return content_list
...
ael_gui_parameters={'runButtonLabel':'&&Run',
                    ...
                    'windowCaption':'HKMOa02 - Credit Limit Monitoring - BK - EF'}
ael_variables=[
['report_name','Report Name','string', None, 'HKMOa02 - Credit Limit Monitoring - BK - EF', 1,0],
['file_path', 'Folder Path', getFilePathSelection(), None, getFilePathSelection(), 1, 1],
['output_file','Output Files','string', ['.pdf', '.xls'], '.pdf', 1 , 0, 'Select Output Extension Type'],
['compliance_rules','Compliance Rule','string', get_compliance_rule_name('Pre-Deal', 'PositionAndRiskControl'), 'BMHK Bank (CEM) DV Limit', 1, 1, 'Compliance Rule with Pre-Deal Category and Position Risk Control Type']
]
...
def ael_main(parameter):
    ...
    title = "HKMOa02 - Credit Limit Monitoring - BK - EF"
    header_list = ["DESCR", "IDCUST", "CCY", "LIMIT", "EXPDATE", "PRODTYPE", "LIMUSD", "EXPUSD", "TERM"]
    content_align = ["mid", "mid", "mid", "mid", "mid", "mid", "mid", "mid", "mid"]

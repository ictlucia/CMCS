    ...
    for compliance_rule in compliance_rules :
        if compliance_rule.Name() == compliance_rule_name:
            for target in compliance_rule.AppliedRules():
                id = target.Target().Id2() if target.Target().Id2() != "" else "-"
                fullName = target.Target().Name() if target.Target().Name() != "" else "-"
                ccy = "IDR"
                limitAmt =  target.ThresholdValues().First().ValueAdjusted() if target.ThresholdValues().First().ValueAdjusted() != "" else "-"
                expDate = target.ThresholdValues().First().AppliedRule().EndDate() if target.ThresholdValues().First().AppliedRule().EndDate() != "" else "-"
                limUsd = round(limitAmt / eodVal, 2)
                content_list.append([id, fullName, ccy, str(limitAmt), expDate, "-", str(limUsd), "-"])
    return content_list

title = "LIMIT ISSUER"
header_list = ["ID", "FullName", "CCY", "LimitAmt", "ExpDate", "ProductType", "LimUSD", "Exposure"]
content_align = ["left", "left", "mid", "right", "mid", "left", "right", "right"]
content_list = get_content_list("Limit - Issuer")
...
html_file = HTMLGenerator().create_html_file(html_code, "C:", "HKMOa05", date_today, False)

    ...
    for compliance_rule in compliance_rules :
        if compliance_rule.Name() == compliance_rule_name:
            for target in compliance_rule.AppliedRules():
                CountryId = target.Target().Description() if target.Target().Description() != "" else "-"
                LongName = target.Target().Name() if target.Target().Name() != "" else "-"
                ccy = "IDR"
                ...
                content_list.append([CountryId, LongName, ccy, expDate, str(limitAmt), "-"])
    return content_list

title = "LIMIT COUNTRY"
header_list = ["CountryId", "LongName", "CCY", "ExpDate", "LimitAmt", "Exposure"]
content_align = ["mid", "left", "mid", "mid", "right", "right"]
content_list = get_content_list("Limit - Country")
...
html_file = HTMLGenerator().create_html_file(html_code, "C:", "HKMOa06", date_today, False)

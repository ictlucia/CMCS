    ...
    for compliance_rule in compliance_rules :
        if compliance_rule.Name() == compliance_rule_name:
            for target in compliance_rule.AppliedRules():
                descr = target.Target().Name() if target.Target().Name() != "" else "-"
                idCust = target.Target().Id2() if target.Target().Id2() != "" else "-"
                ccy = "IDR"
                ...
                content_list.append([descr, idCust, ccy, limit, expDate, "-", limUsd, "-", "-"])
    
    return content_list

title = "LIMIT BANK - EXPIRED"
header_list = ["DESCR", "IDCUST", "CCY", "LIMIT", "EXPDATE", "PRODTYPE", "LIMUSD", "EXPUSD", "TERM"]
content_align = ["left", "left", "mid", "right", "mid", "left", "mid", "mid", "mid"]
content_list = get_content_list("Limit Bank - Expired")
...
html_file = HTMLGenerator().create_html_file(html_code, "C:", "HKMOa01", date_today, False)

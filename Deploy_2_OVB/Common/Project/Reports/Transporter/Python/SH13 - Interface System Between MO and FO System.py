...
def ael_main(parameter):
    ...
    titles = ['', 'STATUS', 'OPICS_SECID', 'SUMMIT_SECID', 'OPICS_PRICE', 'SUMMIT_PRICE', 'OPICS_DATE', 'SUMMIT_DATE']
    ...
    for p in prices:
        if p.Market().Name()=='OPICS':
            for p1 in acm.FInstrument[p.Instrument().Name()].Prices():
                if p1.Market().Name()=='SUMMIT':
                    i += 1
                    if p.Last()==p1.Last():
                        status = 'MATCHED'
                    else:
                        status = 'UNMATCHED'
                    row = [i, status, p.Instrument().Name(), p1.Instrument().Name(), p.Last(), p1.Last(), acm.Time.DateFromTime(p.CreateTime()), acm.Time.DateFromTime(p1.CreateTime())]
                    rows.append(row)

    ...
    for compliance_rule in compliance_rules :
        if compliance_rule.Name() == compliance_rule_name:
            for target in compliance_rule.AppliedRules():
                idCust = target.Target().Id2() if target.Target().HostId() != "" else "-"
...
header_list = ["CIF", "SN", "CCY", "LIMIT", "EXPDATE", "PRODCODE", "TERM", "LIMUSD", "EXPUSD"]
...
html_file = HTMLGenerator().create_html_file(html_code, "C:", "HKMOa03", date_today, False)

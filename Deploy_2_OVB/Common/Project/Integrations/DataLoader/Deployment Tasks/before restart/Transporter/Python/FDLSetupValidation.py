...
class DataloaderSetupValidation():
    ...
    def check_amb_connection(self):
        ...
        try:
            ...
            error_msg = "Unable to connect to AMB with {0}, set in FParameter {1} Exception {2}".format(amb_address, "MESSAGE_BROKER_ex", str(ex))
    ...
    def check_sftp_connection(self):
        ...
        try:        
            ...
            self.logger.info("Unable to import FFTPLibrary. Please import FFTPLibrary in ADS. Error {}".format(str(ex)))
            
        
        if str(obj.sftp.lastErrorText()).__contains__('Received SUCCESS response to subsystem request'):
            self.logger.info("Successfully connected to {} via SFTP".format(self.provider))
            status = 1
        return status
    ...
    def check_webservice_connection(self):
        ...
        try:
            sec_id = "XS2431029441"
            is_connected = FDLWebClient.check_web_connectivity(self.config_variables_obj, sec_id)
            
            if is_connected :
                self.logger.info("Successfully connected to Bloomberg web services")
            else:
                self.logger.error("Cannot connect to Bloomberg web services")
        except Exception as ex:
            self.logger.error("Cannot connect to Bloomberg web services : {}".format(str(ex)) )
            try:
                import zeep
            except Exception as exp:
                self.logger.error("Dataloader has zeep module dependency. Please install zeep module. : {}".format(str(exp)))
    ...
    def check_marketmap_connection(self):
        ...
        try:
            obj = FDLFTPClient.FDLMarketMapClient(self.config_variables_obj)
            if obj.valid_proxy and obj.valid_credentials:
                is_connected = True
        except Exception as ex:
            self.logger.info("Cannot connect to MarketMap, {}".format(str(ex)))
    ...
    def check_data_provider_connection(self):
        ...
        if is_connected:
            ...
            return "Cannot connect to {}".format(self.provider)
            ...
            for channel_name in dl_writer_channels:
                try:
                    ...
                    if handle:
                        ...
                        self.logger.error("Cannot open channels {0} for sending messages to AMB ".format(channel_name))
                        missing_channel.append(channel_name)
                except Exception as e:
                    missing_channel.append(channel_name)
                    self.logger.error("Cannot open channels {0} for writing messages from AMB : {1}".format(channel_name, str(e)))
            ...
            if set_value != None:
                ...
                if param_key == 'BBG_REQUEST_FIELDS_LIST_FILE':
                    ...
                    if type(set_value) == list and (set_value[0] in param_value):
                        ...
                        self.logger.info(valid_msg.format(param_key, set_value))                        
                    elif type(set_value) == str:
                        set_value_list = self.string_as_list(set_value)                        
                        for set_value_in_list in set_value_list :
                            if not (set_value_in_list in param_value):
                                self.logger.error(invalid_msg.format(param_key, set_value_in_list))
                                is_valid = False
                        if is_valid:
                            self.logger.info(valid_msg.format(param_key, set_value))
                    else:
